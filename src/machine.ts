...
function aln(p: Element, x1: number, y1: number, x2: number, y2: number, stroke = RAIL, w = 1.5): void {
  const e = svgEl<SVGLineElement>('line');
  e.setAttribute('x1', String(x1)); e.setAttribute('y1', String(y1));
  e.setAttribute('x2', String(x2)); e.setAttribute('y2', String(y2));
  e.setAttribute('stroke', stroke); e.setAttribute('stroke-width', String(w));
  p.appendChild(e);
}

function acir(p: Element, cx: number, cy: number, r: number, fill: string, stroke?: string, sw = 1): void {
  const e = svgEl<SVGCircleElement>('circle');
  e.setAttribute('cx', String(cx)); e.setAttribute('cy', String(cy)); e.setAttribute('r', String(r));
  e.setAttribute('fill', fill);
  if (stroke) { e.setAttribute('stroke', stroke); e.setAttribute('stroke-width', String(sw)); }
  p.appendChild(e);
}

function apth(p: Element, d: string, fill: string, stroke?: string, sw = 1): void {
  const e = svgEl<SVGPathElement>('path');
...
function gearPath(cx: number, cy: number, r: number, R: number, n: number): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const pt of pts.slice(1)) d += ` L ${pt.join(' ')}`;
  }
  return d + ' Z';
}

function drawGear(parent: Element, cx: number, cy: number, r: number, R: number, teeth: number, dur: number, cw = true): void {
  const g = svgEl<SVGGElement>('g');
  apth(g, gearPath(cx, cy, r, R, teeth), 'none', DECO, 1);
  acir(g, cx, cy, r * 0.42, BG, DECO, 1);
  acir(g, cx, cy, 2, DECO);
  const a = svgEl<SVGAnimateTransformElement>('animateTransform');
...
function drawGearPair(parent: Element, x: number, y: number, onLeft: boolean): void {
  const offset = 13;
...
function drawPulsingDot(parent: Element, x: number, y: number, arrivalT: number, cycleDur: number): void {
  const t1 = +(0.35 / cycleDur).toFixed(6);
  const t2 = +(0.70 / cycleDur).toFixed(6);

  function pulseAnim(target: SVGCircleElement, from: number, to: number): void {
    const a = svgEl<SVGAnimateElement>('animate');
  ...
  const outer = svgEl<SVGCircleElement>('circle');
  outer.setAttribute('cx', String(x)); outer.setAttribute('cy', String(y));
  outer.setAttribute('r', '6'); outer.setAttribute('fill', 'none');
  outer.setAttribute('stroke', DECO); outer.setAttribute('stroke-width', '1');
  pulseAnim(outer, 6, 10);
  parent.appendChild(outer);

  const inner = svgEl<SVGCircleElement>('circle');
  inner.setAttribute('cx', String(x)); inner.setAttribute('cy', String(y));
  inner.setAttribute('r', '2.5'); inner.setAttribute('fill', DECO);
...
function buildBallPath(lx: number, rx: number, crossings: number[], H: number): string {
  const C = 20;
  let d = `M ${lx} 0`;
  let left = true;
...
function findArrivalTime(track: SVGPathElement, targetY: number): number {
...
function drawMachine(svg: SVGSVGElement): void {
  ...
  if (rect.left < MIN_GUTTER) return;
  ...
  const track = svgEl<SVGPathElement>('path');
  track.id = 'track';
  ...
  const DEPART_SEQ = ['ramp',  'gears', 'gears', 'ramp',  'ramp',  'gears'] as const;
  const ARRIVE_SEQ = ['gear',  'dot',   'gear',  'gear',  'dot',   'gear' ] as const;
  ...
  const ball = svgEl<SVGCircleElement>('circle');
  ball.setAttribute('r', '4.5');
  ball.setAttribute('fill', BALL);
  ball.setAttribute('filter', 'url(#glow)');

  const motion = svgEl<SVGAnimateMotionElement>('animateMotion');
  motion.setAttribute('dur', `${cycleDur.toFixed(1)}s`);
  motion.setAttribute('repeatCount', 'indefinite');

  const mpath = svgEl<SVGMPathElement>('mpath');
